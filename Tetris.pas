...
uses
  ...
  Dialogs, GLWin32Viewer, GLCrossPlatform, GLMisc, GLScene, GLObjects,
  GLGraph, GLCadencer,
  //*******************
  TetrisEngine, GLKeyboard, GLTexture, GLSkydome, GLSpaceText, GLScreen,
  GLGameMenu, TGA, GLBitmapFont, GLWindowsFont, GLColor, GLHUDObjects, GLCanvas;
...
begin
  ...
  if Key=Ord('G') then GLXYZGrid1.Visible:=not GLXYZGrid1.Visible;
  if key=VK_UP then UserInput:=inpRotateL;
  if key=VK_DOWN then UserInput:=inpRotateR;
  if key=VK_LEFT then UserInput:=inpMoveL;
  if key=VK_RIGHT then UserInput:=inpMoveR;
  if bloco<>nil then
...
GLHUDSprite1.Position.SetPoint((GLSceneViewer1.Width div 2)+(GLSceneViewer1.Width div 4), 150, 0); 

GLGameMenu1.MaterialLibrary:=GLMaterialLibrary1;
GLGameMenu1.Position.SetPoint(GLSceneViewer1.Width div 2, GLSceneViewer1.Height div 2, 0);
...
IniGame.Position.SetPoint(GLSceneViewer1.Width div 2, GLSceneViewer1.Height div 2, 0);
...
Pause.Position.SetPoint(GLSceneViewer1.Width div 2, GLSceneViewer1.Height div 2, 0);
...
EndGame.Position.SetPoint(GLSceneViewer1.Width div 2, GLSceneViewer1.Height div 2, 0);
...
begin
  ...
  proxbloco:=T3DTBlockViewer(GLDummyCube2.AddNewChild(T3DTBlockViewer));   //cria um novo visualizador
...
var
  GLC : TGLCanvas;
begin
  GLC := TGLCanvas.Create(GLSceneviewer1.Buffer.Width, GLSceneviewer1.Buffer.Height);
